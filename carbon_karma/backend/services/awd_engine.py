"""CarbonKarma — AWD Detection Engine."""
...
from services.time_series import structure_time_series, normalise_features, df_to_tensor
...
def _detect_cycles(fused_records):
    ...
    prev = None; dry_streak = 0; seen_dry = False
    for rec in fused_records:
        ...
        if prev == _FLOODED and state == _DRY:
            seen_dry = True; dry_streak = 0
        if state == _DRY and seen_dry:
            dry_streak += 1
        if prev in (_DRY, _TRANSITION) and state == _FLOODED and seen_dry and dry_streak >= 1:
            n_cycles += 1
            ev = {"timestamp": ts, "water_level": round(wp,4), "rainfall_mm": round(rain,2)}
            (rain_ev if rain >= 10.0 else irr).append(ev)
            seen_dry = False; dry_streak = 0
        prev = state
...
def _prep_lstm_input(fused_records, device):
    rows = [{"timestamp": r["timestamp"], "water_prob_mean": r["water_prob_mean"],
             ...
             "rainfall_norm": r["rainfall_norm"], "vv_mean": r["vv_mean"], "vh_mean": r["vh_mean"]}
...
def detect_awd(fused_records, lstm_model=None, device=None):
    ...
    if not fused_records:
        return {"awd_status":"uncertain","confidence":0.0,"lstm_signal":0.0,"cycles":0,
                "irrigation_events":[],"rain_events":[],"flood_dry_sequence":[],"per_step_status":[]}
    try:
        seq = _prep_lstm_input(fused_records, device)
        lstm_out = run_lstm(lstm_model, seq)
    ...
    elif lstm_signal < 0.35 and n_cycles <= 1: awd_status = "conventional"
    else: awd_status = "uncertain"
    per_step = [{"timestamp": r["timestamp"], "water_level": r["water_level"],
                 "state": _water_state(r["water_level"]), "flood_type": r["flood_type"]}
                for r in fused_records]
    for r in fused_records: r["awd_status"] = awd_status
    return {"awd_status": awd_status, "confidence": _confidence(lstm_signal, n_cycles, len(fused_records)),
